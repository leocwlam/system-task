...
class SystemTask {
  ...
  async preprocessHandler(task, preProcessItem) {
    await task.log('verbose', 'preprocess item', {
      Type: task.type,
      preProcessItem,
    })
  ...
  async cleanupHandler(task, cleanupItems) {
    await task.log('verbose', 'cleanup items', {
      Type: task.type,
      cleanupItems,
    })
    return cleanupItems
  ...
  async start() {
    await this.log('info', `Getting start process under ${this.type}`, {
      Type: this.type,
    })
    try {
      this.isValidProcess()
      ...
      await this.log('info', 'Preprocess all records before execute', {
        Type: this.type,
      })
      ...
      try {
        ...
        await this.log('error', 'Fail: preprocess', {
          Error: error.message,
          Type: this.type,
        })
        throw error
      ...
      await this.log('info', 'Start process', {
        Type: this.type,
        NumberOfPreprocess: preprocessResult.length,
      })
      const processResult = await this.process(preprocessResult)

      // Handle the success process assets
      await this.log('info', 'Cleanup processing after process', {
        Type: this.type,
        NumberOfProcess: processResult.length,
      })
      const cleanupResult = await this.cleanup(processResult)

      await this.log('info', 'Complete: task', {
        Type: this.type,
        NumberOfCleanup: cleanupResult.length,
      })
    } catch (error) {
      await this.log('error', 'Fail: task', {
        Error: error.message,
        Type: this.type,
      })
      throw error
